...
from color_range import get_hsv_values_sky, get_hsv_values_ground,get_max_hsv_values_ground_tilt,get_max_hsv_values_sky_tilt
...
FOV = 48.8
# testing constants
path = 'training_data\sample_images\image9.png'
show_hsv = False #---
show_hsv_mask = False #---
show_bgr2gray = False
show_blue_filtered_greyscale = False
show_blur = False
show_edges = False
show_mask = False #---
show_chain = False
show_hsv_mask_low = False #---
show_hsv_mask_high = False #---
frame_part = 1/7
degree = 0
...
class HorizonDetector:
    ...
    def find_horizon(self, frame: np.ndarray,frame_a, diagnostic_mode: bool = False ):
        """
        ...
        roll, pitch, variance, is_good_horizon = 0, 0, None, None
        if self.predicted_roll is not None:
            degree = self.predicted_roll
        else:
            degree=0
        # get greyscale
        ...
        #finds the HSV range of the sky
        # lH ,lS,lV,hH,hS,hV = get_hsv_values_sky(hsv)
        lH ,lS,lV,hH,hS,hV = get_max_hsv_values_sky_tilt(hsv,degree,frame_part,pitch)
        lower = np.array([lH - Tolarance , lS - Tolarance, lV - Tolarance])
        upper = np.array([hH + Tolarance, hS + Tolarance, hV + Tolarance])
        hsv_mask_high = cv2.inRange(hsv, lower, upper)
        #hsv_mask_high = cv2.bitwise_not(hsv_mask_high)

        #finds the HSV range of the ground
        # lH, lS, lV, hH, hS, hV = get_hsv_values_ground(hsv)
        lH, lS, lV, hH, hS, hV = get_max_hsv_values_ground_tilt(hsv,degree,frame_part,pitch)
        lower = np.array([lH - Tolarance , lS - Tolarance, lV - Tolarance])
        upper = np.array([hH + Tolarance, hS + Tolarance, hV + Tolarance])
        hsv_mask_low = cv2.inRange(hsv, lower, upper)

        #creates a msak with the HSV renage of sky and NOT ground
        hsv_mask_low = cv2.bitwise_not(hsv_mask_low)
        hsv_mask = cv2.bitwise_and(hsv_mask_low,hsv_mask_high)
        #hsv_mask = cv2.bitwise_not(hsv_mask)
        blue_filtered_greyscale = cv2.add(bgr2gray, hsv_mask)

        # generate mask
        blur = cv2.bilateralFilter(blue_filtered_greyscale, 7, 50, 50)
        _, mask = cv2.threshold(blur, 200, 250, cv2.THRESH_OTSU)
        edges = cv2.Canny(image=bgr2gray, threshold1=200, threshold2=250)
        # edges = cv2.Canny(image=hsv , threshold1=200, threshold2=250)
        ...
        if OPERATING_SYSTEM == "Linux":  # for raspberry p	
            #print('got into linux')
            contours, hierarchy = cv2.findContours(mask, cv2.RETR_TREE, chain)
        ...
        if diagnostic_mode:
            ...
            if self.predicted_roll:
                ...
                cv2.putText(mask, 'Horizon Lock', (20, 40), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 150, 255), 1,
                            cv2.LINE_AA)
            ...
            if (show_edges):
                ...
                cv2.waitKey(1)
            if (show_blue_filtered_greyscale):
            ...
            if (show_hsv_mask):
                ...
                cv2.waitKey(1)
            if (show_hsv):
                hsv = cv2.resize(hsv, desired_dimensions)
                cv2.imshow('hsv', hsv)
                cv2.waitKey(1)
            if (show_bgr2gray):
            ...
            if (show_blur):
                ...
                cv2.waitKey(1)
            if (show_chain):
                chain = cv2.resize(chain, desired_dimensions)
                cv2.imshow('chain', chain)
                cv2.waitKey(1)
            if (show_hsv_mask_low):
                hsv_mask_low = cv2.resize(hsv_mask_low, desired_dimensions)
                cv2.imshow('hsv_mask_low', hsv_mask_low)
                cv2.waitKey(1)
            if (show_hsv_mask_high):
                hsv_mask_high = cv2.resize(hsv_mask_high, desired_dimensions)
                cv2.imshow('hsv_mask_high', hsv_mask_high)
                cv2.waitKey(1)
...
if __name__ == "__main__":
    ...
    for n in range(ITERATIONS):
        ...
        frame_small = crop_and_scale(frame, **CROP_AND_SCALE_PARAM)
        output = horizon_detector.find_horizon(frame_small,frame, diagnostic_mode=False)
